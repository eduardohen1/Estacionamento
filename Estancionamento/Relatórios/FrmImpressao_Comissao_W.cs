...
{
    public partial class FrmImpressao_Comissao_W : Form
    {
        FrmPrincipal vTelaPrincipal;
        string sArquivoConexao;
        Veiculos cVeiculos;

        public FrmImpressao_Comissao_W(FrmPrincipal vvTelaPrincipal, string ssArquivoConexao)
        {
            InitializeComponent();
        ...
        private void FrmImpressao_Comissao_W_Load(object sender, EventArgs e)
        {
            cVeiculos = new Veiculos();
            cVeiculos.ArquivoConexao = sArquivoConexao;
            DataSet dsTemp = cVeiculos.impr_Comissao(0);
            if (dsTemp != null)
            {
